...
class Damage extends React.Component<Props> {
  ...
  stopSign({
    ...
    for (let i = angle; i < 2 * Math.PI; i += 2 * angle) {
      points += `${x + radius * Math.cos(i)}, ${y + radius * Math.sin(i)} `
    }
    return (
      <g key={`damage-${x}-${y}`} className={'StopSign'}>
        <polygon points={points} />
        <text x={x} y={y - radius / 5} textAnchor={'middle'} style={stopSignTextStyle}>
          {text0}
        </text>
        <text x={x} y={y + (2 * radius) / 5} textAnchor={'middle'} style={stopSignTextStyle}>
          {text1}
        </text>
      </g>
    )
