class VehicleStatusHelper {
  static driverAwake(vehicle) {
    return (
      vehicle.design.components.crew.find((member) => member.role === 'driver')
        .damagePoints > 1
    )
  }

  static numberOfWheels(vehicle) {
    return vehicle.design.components.tires.filter(
      (filtire) => filtire.wheelExists,
    ).length
  }
  ...
  static canSteer(vehicle) {
    return (
      VehicleStatusHelper.driverAwake(vehicle) &&
      VehicleStatusHelper.enoughWheels(vehicle)
    )
  }
